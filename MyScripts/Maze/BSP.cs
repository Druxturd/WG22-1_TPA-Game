...
public class BSP : MonoBehaviour
{
    ...
    public class Node
    {
        ...
        public bool CheckRatio(int Width, int Height, int NewWidth, int NewHeight)
        {
            ...
            {
                if (
                    NewHeight * 1f / Width < MaxRatio
                    || (Height - NewHeight) * 1f / Width < MaxRatio
                ) return false;
            }
            else
            {
                if (
                    NewWidth * 1f / Height < MaxRatio
                    || (Width - NewWidth) * 1f / Height < MaxRatio
                ) return false;
            }
            return true;
        }
    ...
    public void MovePlayerToStartMaze()
    {
        ...
        GameObject.Find("Player").transform.position = new Vector3(StartMaze.x, StartMaze.y, StartMaze.z);
